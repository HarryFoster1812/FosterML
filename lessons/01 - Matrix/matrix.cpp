...
Matrix<T> Matrix<T>::operator*(const Matrix<T>& other) const {
    if (cols != other.rows)
        throw std::invalid_argument("Matrix multiplication is not defined for the given matrices");
...
Matrix<T> Matrix<T>::randomMatrix(int rows, int cols, T minVal, T maxVal){
    ...
    if constexpr (std::is_integral<T>::value) { // check for integer type on compile time
       std::uniform_int_distribution<T> dist(minVal, maxVal);  // use an integer distribution
        for (int i = 0; i < rows; ++i)
            for (int j = 0; j < cols; ++j)
                result(i, j) = dist(gen);
    }

    else if constexpr (std::is_floating_point<T>::value) { // check for floating types on compile time
