...
TensorPtr<T> Tensor<T>::divide(const TensorPtr<T> &other) const {
  ...
  try {
    ...
    backward_op = [this_ptr = TensorPtr<T>(this_shared),
      ...
      if (other_ptr->requiresGrad()) {
        TensorPtr<T> grad_B = grad->multiply(this_ptr)->negate(); // grad * -A
        TensorPtr<T> B_squred = other_ptr->multiply(other_ptr);   // grad * -A
...
template <typename T, typename Op>
TensorPtr<T>
elementwise_op(const TensorPtr<T> &A, Op op,
               std::function<void(const Tensor<T> &)> backward_op) {
  // Broadcast shapes

  // Create result tensor
  TensorPtr<T> result = TensorPtr<T>::create(A->getShape(), A->requiresGrad());
...
TensorPtr<T> Tensor<T>::exp() const // element-// element-wise exponentiation
{
  ...
  try {
    ...
    backward_op = [a_ptr = this_shared](const Tensor<T> &result) {
      ...
      a_ptr->addGrad(grad->multiply(TensorPtr<T>(result.shared_from_this())));
    };
  } catch (const std::bad_weak_ptr &e) {
    std::cerr << "BAD WEAK PTR IN UNARAY BACKWARDS FUNCTION" << std::endl;
  }
...
TensorPtr<T> Tensor<T>::sigmoid() const // element-// element-wise sigmoid
{
  ...
  try {
    ...
    backward_op = [a_ptr = this_shared](const Tensor<T> &result) {
      ...
      const TensorPtr<T> scaling_tensor = result.multiply(result.subtract(1));
...
TensorPtr<T> Tensor<T>::matrixmul(const TensorPtr<T> &other) const {
  ...
  if (result->requiresGrad()) {
    ...
    result->setBackwardFunction(
        [a_ptr = TensorPtr<T>(this_shared), b_ptr = TensorPtr<T>(other_shared),
         ...
         B_bcast = TensorPtr<T>(Bb_shared)](const Tensor<T> &result) {
          ...
          if (a_ptr->requiresGrad()) {
            ...
            TensorPtr<T> dA =
                Tensor<T>::sum_over_broadcasted_axes(dA_raw, a_ptr->getShape());
            a_ptr->addGrad(dA);
          }
          ...
          if (b_ptr->requiresGrad()) {
            ...
            TensorPtr<T> dB =
                Tensor<T>::sum_over_broadcasted_axes(dB_raw, b_ptr->getShape());
            b_ptr->addGrad(dB);
          }
        });
    // add the parents
